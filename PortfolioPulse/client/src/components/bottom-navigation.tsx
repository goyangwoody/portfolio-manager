...
export function BottomNavigation() {
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 bg-white dark:bg-dark-card border-t border-gray-200 dark:border-gray-700 px-2 py-1">
          ...
          {navigationItems.map((item) => {
            ...
            return (
              <Link
                key={item.path}
                href={item.path}
                className={`flex flex-col items-center justify-center py-2 px-2 rounded-lg transition-colors duration-200 min-h-[50px] w-[60px] ${
                  isActive
